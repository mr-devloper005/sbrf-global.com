    ...
    <EditableSiteShell>
      <main className="px-4 py-14 sm:px-6 lg:px-8">
        <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          <article className="rounded-[6px] border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] p-8 lg:p-12">
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[var(--slot4-accent)]">{pagesContent.about.badge}</p>
            <h1 className="editable-display mt-5 text-5xl font-semibold tracking-0">About {SITE_CONFIG.name}</h1>
          ...
          <aside className="space-y-4">
            {pagesContent.about.values.map((value) => (
              <div key={value.title} className="rounded-[6px] border border-[var(--editable-border)] bg-[var(--slot4-panel-bg)] p-6">
